 ...
 *
 * Intermediate planes (rgb_d0..3, gmin/gmax_buf, drv0..3) use DemosaicPlane:
 * F16 storage on ARM64 / F32 elsewhere. Arithmetic is always F32 — reads
 * upcast via operator[], writes downcast via set(). Input RawMosaic::data and
 * output ImageBuffer::data stay float32; conversion happens at the
 * populate/write-output boundaries automatically.
 *
 * Constants (xtrans_demosaic.py:24-25): BORDER=12, NDIR=4.
 *
 * Memory: peaks at ~19 x H x W x 4 bytes (4 dir buffers x 3 ch + 4 drv
 * planes + 1 out; F16 storage on ARM64 halves this). Intermediates are freed
 * in the same order as the reference's `del` statements
 * (xtrans_demosaic.py:860, 869, 880, 897-898).
 */
...
static void xtmPopulate(const RawMosaic& m,
                        DemosaicPlane& rgb_d0,
                        DemosaicPlane& rgb_d1,
                        DemosaicPlane& rgb_d2,
                        DemosaicPlane& rgb_d3) {
    const int W = m.width;
    ...
    for (int row = 0; row < H; ++row) {
        for (int col = 0; col < W; ++col) {
            ...
            rgb_d0.set(i3(row, col, 0, W), 0.0f);
            rgb_d0.set(i3(row, col, 1, W), 0.0f);
            rgb_d0.set(i3(row, col, 2, W), 0.0f);
            rgb_d0.set(i3(row, col, f, W), val);
        }
    }

    // Copy d0 -> d1, d2, d3 (xtrans_demosaic.py:131-136). Element-wise so no
    // raw pointer touches the (F16 on ARM64) storage.
    const int n3 = static_cast<int>(static_cast<size_t>(H) * W * 3);
    #ifdef RA_USE_OPENMP
    #pragma omp parallel for schedule(static)
    #endif
    for (int i = 0; i < n3; ++i) {
        const float v = rgb_d0[i];
        rgb_d1.set(i, v);
        rgb_d2.set(i, v);
        rgb_d3.set(i, v);
    }
...
// BORDER PRE-FILL
// ============================================================
// fillBorderIntermediates12: fill the kBORDER=12 ring of all 4 direction
// buffers (all 3 channels) with a 3x3 bilinear estimate from the raw mosaic
// by FCxt color. darktable avoids cascade contamination from zero-initialized
// border non-native CFA positions via CLAMP_TO_EDGE; pre-filling the border
// ring mirrors that. Applied after populate, before gminmax.
static void fillBorderIntermediates12(const RawMosaic& m,
    ...
    // Per-pixel 3x3 bilinear estimate into all 4 direction buffers at a
    // border position (all 3 channels).
    auto body = [&](int row, int col) {
        ...
        for (int c = 0; c < 3; ++c) {
            rgb_d0.set(i3(row, col, c, W), est[c]);
            rgb_d1.set(i3(row, col, c, W), est[c]);
            rgb_d2.set(i3(row, col, c, W), est[c]);
            rgb_d3.set(i3(row, col, c, W), est[c]);
        }
    };
...
static void xtmGminmax(const DemosaicPlane& rgb_d0,
                       DemosaicPlane& gmin_buf,
                       DemosaicPlane& gmax_buf,
                       const AllHex& hex,
                       const RawMosaic& m) {
    const int W = m.width;
    const int H = m.height;
    ...
    for (int row = pad; row < H - pad; ++row) {
        for (int col = pad; col < W - pad; ++col) {
            ...
            for (int k = 0; k < 6; ++k) {
                ...
                const float val = rgb_d0[i3(row + dr, col + dc, 1, W)];
                mn = std::min(mn, val);
                mx = std::max(mx, val);
            }
...
static inline float greenInterpOne(const DemosaicPlane& d0,
                                   const AllHex& hex,
                                   const RawMosaic& m,
                                   int row, int col,
                                   int color_idx) {
...
static void xtmGreenInterp(DemosaicPlane& rgb_d0,
                           DemosaicPlane& rgb_d1,
                           DemosaicPlane& rgb_d2,
                           DemosaicPlane& rgb_d3,
                           const DemosaicPlane& gmin_buf,
                           const DemosaicPlane& gmax_buf,
                           const AllHex& hex,
                           const RawMosaic& m) {
    const int W = m.width;
    const int H = m.height;
    ...
    for (int row = pad; row < H - pad; ++row) {
        for (int col = pad; col < W - pad; ++col) {
            ...
            const size_t i = static_cast<size_t>(row) * W + col;
            const float mn = gmin_buf[i];
            const float mx = gmax_buf[i];

            const float c0 = greenInterpOne(rgb_d0, hex, m, row, col, 0);
            const float c1 = greenInterpOne(rgb_d0, hex, m, row, col, 1);
            const float c2 = greenInterpOne(rgb_d0, hex, m, row, col, 2);
            const float c3 = greenInterpOne(rgb_d0, hex, m, row, col, 3);
            ...
            rgb_d0.set(i3(row, col, 1, W), vals[0 ^ flip]);
            rgb_d1.set(i3(row, col, 1, W), vals[1 ^ flip]);
            rgb_d2.set(i3(row, col, 1, W), vals[2 ^ flip]);
            rgb_d3.set(i3(row, col, 1, W), vals[3 ^ flip]);
        }
    }
}
...
static inline void rbSolitaryGreenForDir(const DemosaicPlane& rgb,
...
static void xtmRbAtGreen(DemosaicPlane& rgb_d0,
                         DemosaicPlane& rgb_d1,
                         DemosaicPlane& rgb_d2,
                         DemosaicPlane& rgb_d3,
    ...
    for (int row = pad; row < H - pad; ++row) {
        for (int col = pad; col < W - pad; ++col) {
            ...
            rbSolitaryGreenForDir(rgb_d0, row, col, W, h0, 0, r, b);
            rgb_d0.set(i3(row, col, 0, W), r);
            rgb_d0.set(i3(row, col, 2, W), b);

            rbSolitaryGreenForDir(rgb_d1, row, col, W, h0, 1, r, b);
            rgb_d1.set(i3(row, col, 0, W), r);
            rgb_d1.set(i3(row, col, 2, W), b);

            rbSolitaryGreenForDir(rgb_d2, row, col, W, h0, 2, r, b);
            rgb_d2.set(i3(row, col, 0, W), r);
            rgb_d2.set(i3(row, col, 2, W), b);

            rbSolitaryGreenForDir(rgb_d3, row, col, W, h0, 2, r, b);
            rgb_d3.set(i3(row, col, 0, W), r);
            rgb_d3.set(i3(row, col, 2, W), b);
        }
    }
}
...
static inline float rbCrossForDir(const DemosaicPlane& rgb,
                                  int row, int col, int W,
...
static void xtmRbInterp(DemosaicPlane& rgb_d0,
                        DemosaicPlane& rgb_d1,
                        DemosaicPlane& rgb_d2,
                        DemosaicPlane& rgb_d3,
                        const RawMosaic& m, const AllHex& hex) {
    ...
    for (int row = pad; row < H - pad; ++row) {
        for (int col = pad; col < W - pad; ++col) {
            ...
            rgb_d0.set(i3(row, col, f, W), rbCrossForDir(rgb_d0, row, col, W, f, sgrow, 0));
            rgb_d1.set(i3(row, col, f, W), rbCrossForDir(rgb_d1, row, col, W, f, sgrow, 1));
            rgb_d2.set(i3(row, col, f, W), rbCrossForDir(rgb_d2, row, col, W, f, sgrow, 2));
            rgb_d3.set(i3(row, col, f, W), rbCrossForDir(rgb_d3, row, col, W, f, sgrow, 3));
        }
    }
}
...
static inline void fillG22ForDir(DemosaicPlane& rgb,
                                 const AllHex& hex,
                                 int row, int col, int W,
    ...
    if (sum_nonzero) {
        ...
        for (int ch = 0; ch <= 2; ch += 2) {
            rgb.set(i3(row, col, ch, W), (g + 2.0f * rgb[i3(row + dra, col + dca, ch, W)]
        ...
        const float g = (2.0f * rgb[i3(row, col, 1, W)]
                       ...
                       - rgb[i3(row + drb, col + dcb, 1, W)]);
        for (int ch = 0; ch <= 2; ch += 2) {
            rgb.set(i3(row, col, ch, W), (g + rgb[i3(row + dra, col + dca, ch, W)]
...
static void xtmFillGreen22(DemosaicPlane& rgb_d0,
                           DemosaicPlane& rgb_d1,
                           const AllHex& hex,
                           const RawMosaic& m) {
    const int W = m.width;
    ...
    for (int row = pad; row < H - pad; ++row) {
        for (int col = pad; col < W - pad; ++col) {
            ...
            fillG22ForDir(rgb_d0, hex, row, col, W, 0, 1);
            fillG22ForDir(rgb_d1, hex, row, col, W, 2, 3);
        }
    }
}
...
static void xtmYuvDerivatives(const DemosaicPlane& rgb_d0,
                              const DemosaicPlane& rgb_d1,
                              const DemosaicPlane& rgb_d2,
                              const DemosaicPlane& rgb_d3,
                              DemosaicPlane& drv0,
                              DemosaicPlane& drv1,
                              DemosaicPlane& drv2,
                              DemosaicPlane& drv3,
    ...
    for (int row = pad; row < H - pad; ++row) {
        for (int col = pad; col < W - pad; ++col) {
            ...
            const DemosaicPlane* rgb_dir[4] = {&rgb_d0, &rgb_d1, &rgb_d2, &rgb_d3};
            DemosaicPlane*       drv_dir[4] = {&drv0,   &drv1,   &drv2,   &drv3};

            for (int d_idx = 0; d_idx < 4; ++d_idx) {
                const int dr = dr_tab[d_idx];
                const int dc = dc_tab[d_idx];
                const DemosaicPlane* rx = rgb_dir[d_idx];
                ...
                drv_dir[d_idx]->set(static_cast<size_t>(row) * W + col, deriv);
            }
        }
    }
}
...
static void xtmHomoMerge(const DemosaicPlane& rgb_d0,
                         const DemosaicPlane& rgb_d1,
                         const DemosaicPlane& rgb_d2,
                         const DemosaicPlane& rgb_d3,
                         const DemosaicPlane& drv0,
                         const DemosaicPlane& drv1,
                         const DemosaicPlane& drv2,
                         const DemosaicPlane& drv3,
                         ImageBuffer& out) {
    const int W = out.width;
    const int H = out.height;
    const int pad = kBORDER;

    const DemosaicPlane* drv_dir[4] = {&drv0, &drv1, &drv2, &drv3};
    const DemosaicPlane* rgb_dir[4] = {&rgb_d0, &rgb_d1, &rgb_d2, &rgb_d3};
        ...
        for (int col = pad; col < W - pad; ++col) {
            ...
            for (int vv = -2; vv <= 2; ++vv) {
                for (int hh = -2; hh <= 2; ++hh) {
                    ...
                    for (int di = 0; di < 4; ++di) {
                        const DemosaicPlane* drv_di = drv_dir[di];
                        int cnt = 0;
            ...
            for (int di = 0; di < 4; ++di) {
                if (homosum[di] >= threshold) {
                    const DemosaicPlane* rx = rgb_dir[di];
            ...
            if (cnt > 0.0f) {
                ...
                px[0] = std::max(0.0f, rgb_d0[i3(row, col, 0, W)]);
                px[1] = std::max(0.0f, rgb_d0[i3(row, col, 1, W)]);
                px[2] = std::max(0.0f, rgb_d0[i3(row, col, 2, W)]);
            }
        }
    }
}
...
ImageBuffer xtransMarkesteijnDemosaic(const RawMosaic& m) {
    ...
    // 4 direction buffers, each (H, W, 3) — interleaved RGB (F16 storage on
    // ARM64, F32 elsewhere).
    DemosaicPlane rgb_d0(N3), rgb_d1(N3), rgb_d2(N3), rgb_d3(N3);
    ...
    DemosaicPlane gmin_buf(N), gmax_buf(N);
    ...
    DemosaicPlane drv0(N), drv1(N), drv2(N), drv3(N);
